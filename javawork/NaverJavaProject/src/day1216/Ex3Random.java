...
public class Ex3Random {
	...
	public static void main(String[] args) {
		 ...
		 *     y가 아닐경우 프로그램 종료
		 * (이런경우 숫자 입력하다가 문자 입력해야한다 (문자 입력후 숫자 입력은 가능함))
		 */
			...
			while(true)
			{
				...
				while(true)
				{
					...
					if(su>rnd)
						...
						System.out.println("숫자 맞추기 게임을 계속 하려면 y, 그만하려면 n");//여기에 값이 없다보니 charAt에서 에러 발생
						char ch=sc.nextLine().charAt(0);
						if(ch=='y' || ch=='Y') {
							System.out.println("새로운 난수를 발생합니다");
							//break;//현재 속한 while문만 빠져나간다
							continue Exit; //바깥쪽 반복문의 조건식으로 이동(무조건 첫줄부터 실행됨)
						}else {
